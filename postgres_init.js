...
client.connect().then(async () => {
    try {
        console.log('creating postgreSQL database schema') //create vacations table
        await client.query(createScript)
        const vacationCount = await getVacationCount(client)
        if(vacationCount === 0) {
            console.log('PostgreSQL: vacation count is 0. seeding vacations...')
            await seedVacations(client)
        }
    }catch(err) {
        console.log('ERROR: could not initialize postgreSQL database')
