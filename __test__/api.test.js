...
//open bash and switch to the working directory
//type in npm test api.test
...
describe('API tests', () => {
    ...
    test('POST /api/vacation/:email/:sku/notify-when-in-season', async() => {
        const vacations = await _fetch('get', '/api/vacations')
        expect(vacations.length).not.toBe(0)
        const vacation0 = vacations[0]
        const email = 'warrior1744@gmail.com'
        await _fetch('post', `/api/vacation/${email}/${vacation0.sku}/notify-when-in-season`,
